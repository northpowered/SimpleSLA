...
class SLAConfig:
    ...
    def _load_config(self, config_from_file):
        ...
        parsed_configuration["server"]["bind_address"] = config_data.get("server").get(
            "bind_address", "127.0.0.1"
        )
        parsed_configuration["server"]["port"] = config_data.get("server").get(
            "port", "8800"
        )
        parsed_configuration["server"]["refresh_time"] = config_data.get("server").get(
            "refresh_time", "1"
        )
        ...
        for raw_device in config_data.get("devices", list()):
            ...
            try:
                ...
                if not isinstance(device["name"], str):
                    LG.error(
                        "Device name >> {name} << should be String type",
                        name=device["name"],
                    )
            ...
            try:
                ...
                if not isinstance(device["type"], str):
                    LG.error(
                        "Device type >> {type} << should be String type",
                        type=device["type"],
                    )
            ...
            try:
                ...
                if not isinstance(device["transport"], str):
                    LG.error(
                        "Device transport >> {transport} << should be String type",
            ...
            try:
                ...
                if not isinstance(device["address"], str):
                    LG.error(
                        "Device address >> {address} << should be String type",
            ...
            try:
                ...
                if not isinstance(device["username"], str):
                    LG.error(
                        "Device username >> {username} << should be String type",
            ...
            try:
                ...
                if not isinstance(device["password"], str):
                    LG.error(
                        "Device password >> {password} << should be String type",
            ...
            try:
                ...
                if not isinstance(device["port"], int):
                    LG.error(
                        "Device port >> {port} << should be Int type",
                        port=device["port"],
                    )
            ...
            parsed_configuration["devices"][local_device["name"]] = Device(
                **local_device
            )
        ...
        for raw_policy in config_data.get("policies", list()):
            ...
            try:
                ...
                if not isinstance(policy["name"], str):
                    LG.error(
                        "Policy name >> {name} << should be String type",
                        name=policy["name"],
                    )
            ...
            try:
                ...
                if not isinstance(policy["max_rtt"], (int, float)):
                    LG.error(
                        "Policy max_rtt >> {max_rtt} << should be Int or Float type",
        ...
        for raw_service in config_data.get("services", list()):
            ...
            try:
                ...
                if not isinstance(service["name"], str):
                    LG.error(
                        "Service name >> {name} << should be String type",
                        name=service["name"],
                    )
            ...
            try:
                ...
                if not isinstance(_, str):
                    LG.error(
                        "Service device >> {device} << should be String type", device=_
                    )
                try:
                    service["device"] = parsed_configuration["devices"][_]
                except KeyError:
                    LG.error(
                        "Unknown device {device} in {service} service",
                        device=_,
            ...
            try:
                ...
                if not isinstance(service["target"], str):
                    LG.error(
                        "Service target >> {target} << should be String type",
            ...
            try:
                ...
                if not isinstance(_, str):
                    LG.error(
                        "Service policy >> {policy} << should be String type", policy=_
                    )
                try:
                    service["policy"] = parsed_configuration["policies"][_]
                except KeyError:
                    LG.error(
                        "Unknown policy {policy} in {service} service",
                        policy=_,
                        service=service["name"],
                    )
            ...
            try:
                ...
                if not isinstance(service["delay"], int):
                    LG.error(
                        "Service delay >> {delay} << should be Int type",
                        type=service["delay"],
                    )
